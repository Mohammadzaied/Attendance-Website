...
export default function DHSubjectStudentsPage() {
  ...
  return (
    ...
    >
        ...
        <div className="text-right">
          <div className="flex items-center gap-2 mb-2">
            <Button
              onClick={() => router.back()}
              variant="outline"
              className="w-fit self-start md:self-center gap-2 rounded-xl font-bold cursor-pointer"
            >
          ...
          <p className="text-sm md:text-base text-gray-600 font-medium italic">
            عرض سجلات الغياب لكل طالب (رئيس قسم)
          </p>
        </div>
      </div>

      {/* Absence Statistics Button */}
      <Card className="shadow-sm border-blue-100 bg-linear-to-br from-blue-50 to-white overflow-hidden rounded-2xl">
        <CardContent className="p-5 md:p-6">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            ...
            <Button
              ...
              className="w-full md:w-auto bg-blue-600 hover:bg-blue-700 text-white gap-2 h-11 font-black rounded-xl shadow-lg shadow-blue-600/20 cursor-pointer transition-all"
            >
      ...
      {/* Search and Stats */}
      <Card className="shadow-sm border-gray-100 overflow-hidden rounded-2xl bg-white">
        <CardContent className="p-5 md:p-6">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div className="relative flex-1 max-w-md">
              ...
              <Input
                ...
                className="pr-10 text-right h-11 rounded-xl font-bold"
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
              />
            </div>
            <div className="flex items-center gap-3 self-end md:self-center">
              <Badge className="bg-blue-50 text-blue-700 border-blue-200 px-4 py-2 text-sm font-black rounded-xl">
      ...
      {fetchStudentsState.isLoading ? (
        <div className="flex flex-col items-center justify-center py-20 text-gray-400 gap-4">
          ...
          <p className="font-bold text-lg">جاري تحميل قائمة الطلاب...</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredStudents.map((student) => (
            <Link
              ...
              href={`/department-head/subjects/student-absences/${subjectId}/${student.studentAcademicInfoId}`}
              className="block group"
            >
              <Card className="h-full border-gray-100 group-hover:border-blue-300 group-hover:shadow-md transition-all duration-200 rounded-2xl">
                <CardContent className="p-5 flex items-center gap-4">
                  <div className="h-12 w-12 rounded-full bg-linear-to-br from-blue-500 to-indigo-600 flex items-center justify-center text-white font-black text-lg shrink-0 shadow-sm group-hover:scale-110 transition-transform">
                    {student.fullName.charAt(0)}
                  </div>
                  <div className="flex-1 min-w-0 text-right">
                    <p className="font-black text-gray-900 group-hover:text-blue-700 transition-colors truncate">
                      {student.fullName}
                    </p>
